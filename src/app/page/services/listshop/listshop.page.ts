...
export class ListshopPage implements OnInit {
 ...
 console.log(this.DataRequest)
   this.server.GetLojasServicesServices(this.DataRequest).then( (data:any)=>{
    ...
    buttons: [
      {
        ...
        handler:()=>{
          this.ngZone.run(() =>
          this.router.navigateBack('home/services/mascote/'+this.DataRequest.id_especie+'/'+this.DataRequest.categorie+'/'+data.id)
          );
        },
        ...
        handler:()=>{
          this.ngZone.run(() =>
          this.router.navigateBack('home/red/loja/perfil-loja/'+data.id)
          );
        },
        icon:'paw-outline'
        }
